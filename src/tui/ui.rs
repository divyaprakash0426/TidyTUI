...
pub fn ui(f: &mut Frame, app: &mut App) {
    ...
    // --- Content ---
    match app.app_state {
        AppState::Viewing => match app.active_tab {
...
fn render_dashboard(f: &mut Frame, app: &App, area: Rect) {
    ...
    f.render_widget(
        legend,
        sub_chunks[1].inner(ratatui::layout::Margin {
            horizontal: 1,
            vertical: 1,
        }),
    );
    ...
    f.render_widget(
        gauge,
        sub_chunks[0].inner(ratatui::layout::Margin {
            horizontal: 1,
            vertical: 1,
        }),
    );
...
fn render_results(f: &mut Frame, app: &mut App, area: Rect) {
    let list_items: Vec<ListItem> = app
        ...
        .iter()
        .map(|row| match row {
            ResultRow::CategoryHeader(cat) => ListItem::new(Line::from(vec![Span::styled(
                format!("── {} ──", cat),
                Style::default()
                    .fg(Color::DarkGray)
                    .add_modifier(Modifier::BOLD),
